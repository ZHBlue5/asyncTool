package com.zhblue.platform.wrapper;
...
import com.zhblue.platform.callback.DefaultCallback;
...
import com.zhblue.platform.exception.SkippedException;
...
public class WorkerWrapper<T, V> {
     ...
     * 依赖的wrappers，有2种情况，
     * <p>
     * 1:必须依赖的全部完成后，才能执行自己 2:依赖的任何一个、多个完成了，就可以执行自己 通过must字段来控制是否依赖项必须完成
     *
     */
    private List<DependWrapper> dependWrappers;
    /**
     * 标记该事件是否已经被处理过了，譬如已经超时返回false了， volatile并不能保证"同一毫秒"内,多线程对该值的修改和拉取
     * <p>
     ...
     *            线程池
     * @param fromWrapper
     *            来源wrapper：fromWrapper代表这次work是由哪个上游wrapper发起的
     * @param remainTime
     *            总超时时间； null则不超时
    ...
    private void work(ExecutorService executorService, WorkerWrapper fromWrapper, Long remainTime,
        ...
        // 若设置了超时时间
        // 总的已经超时了，就快速失败，进行下一个
        if (Objects.nonNull(remainTime) && remainTime <= 0) {
            fastFail(INIT, null);
        ...
        // 如果自己已经执行过了。
        // 可能有多个依赖，其中的一个依赖已经执行完了，并且自己也已开始执行或执行完毕。
    ...
    public void work(ExecutorService executorService, Map<String, WorkerWrapper> forParamUseWrappers) {
        work(executorService, null, null, forParamUseWrappers);
    }
    ...
    private void beginNext(ExecutorService executorService, long now, Long remainTime) {
        ...
        if (nextWrappers.size() == 1) {
            ...
            nextWrappers.get(0).work(executorService, WorkerWrapper.this,
                Objects.nonNull(remainTime) ? remainTime - costTime : null, forParamUseWrappers);
            return;
        }
        ...
        for (int i = 0; i < nextWrappers.size(); i++) {
            ...
            futures[i] =
                CompletableFuture.runAsync(
                    () -> nextWrappers.get(finalI).work(executorService, WorkerWrapper.this,
                        Objects.nonNull(remainTime) ? remainTime - costTime : null, forParamUseWrappers),
                    executorService);
        }
        try {
    ...
    public boolean equals(Object o) {
        ...
        return needCheckNextWrapperResult == that.needCheckNextWrapperResult && Objects.equals(param, that.param)
            && Objects.equals(worker, that.worker) && Objects.equals(callback, that.callback)
            && Objects.equals(nextWrappers, that.nextWrappers) && Objects.equals(dependWrappers, that.dependWrappers)
